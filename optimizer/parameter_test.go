...
func Test_NewParameter(t *testing.T) {
	...
	source = mustMatrix(t, 1, 3, []float64{1, 2, 3})
	...
	testutil.RequireSliceAlmostEqual(t, values, []float64{1, 2, 3}, epsilon)
	...
	testutil.RequireSliceAlmostEqual(t, gradientValues, []float64{0, 0, 0}, epsilon)
...
func Test_Parameter_AccumulateGradient(t *testing.T) {
	...
	firstGradient = mustMatrix(t, 1, 3, []float64{0.1, 0.2, 0.3})
	secondGradient = mustMatrix(t, 1, 3, []float64{0.4, 0.5, 0.6})
	...
	testutil.RequireSliceAlmostEqual(t, values, []float64{0.5, 0.7, 0.9}, epsilon)
}
...
func Test_Parameter_AccumulateGradient_ValidatesShape(t *testing.T) {
	...
	parameter = mustParameter(t, 1, 3, []float64{1, 2, 3})
	gradient = mustMatrix(t, 1, 2, []float64{0.1, 0.2})
...
func Test_Parameter_ResetGradient(t *testing.T) {
	...
	parameter = mustParameter(t, 1, 3, []float64{1, 2, 3})
	gradient = mustMatrix(t, 1, 3, []float64{0.1, 0.2, 0.3})
	...
	testutil.RequireSliceAlmostEqual(t, values, []float64{0, 0, 0}, epsilon)
}

func mustParameter(tb testing.TB, rows, cols int, values []float64) (parameter *optimizer.Parameter) {
	var (
